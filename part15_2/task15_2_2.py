...
# Если догадка меньше случайного числа, то программа должна вывести сообщение 'Слишком мало,
# попробуйте еще раз'. Если пользователь угадывает число, то программа должна поздравить
...
# Пользователь потенциально может ввести неверные данные, например, не число, или число превышающее 100. Важно предусмотреть такую возможность,
# чтобы программа продолжала правильно работать. Обработка такого рода ситуаций называется защитой от дурака.
# Напишите функцию is_valid() в которую передается один строковый аргумент. Функция возвращает значение True если переданный аргумент является
# целым числом от 1 до 100 и False в противном случае
...
while True:
    max_num_input = input('Введите максимальное число для угадывания (больше 1): ').strip()
    if max_num_input.isdigit() and int(max_num_input) > 1:
        max_num = int(max_num_input)
    else:
    ...
    while True:
        guess = input(f'Введите число от 1 до {max_num}: ').strip()
